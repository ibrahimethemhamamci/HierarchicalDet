...
def transform_instance_annotations(
    annotation, transforms, image_size, *,keypoint_hflip_indices=None,  bbox_pre=None, 
):
    """
   ...
   # print(annotation["bbox"])
    if bbox_pre!=None:
      bbox_pre_final=[]
      for box_pre in bbox_pre:    
        box_pre = BoxMode.convert(box_pre, BoxMode.XYWH_ABS, BoxMode.XYXY_ABS)
        # clip transformed bbox to image size
        
        box_pre = transforms.apply_box(np.array([box_pre]))[0].clip(min=0)
        box_pre=np.minimum(box_pre, list(image_size + image_size)[::-1])
        bbox_pre_final.append(box_pre)
      
      annotation["bbox_pre"] = bbox_pre_final
      annotation["bbox"] = np.minimum(bbox, list(image_size + image_size)[::-1])
      annotation["bbox_mode"] = BoxMode.XYXY_ABS  
    #print(annotation["bbox_pre"])
...
def annotations_to_instances(annos, image_size, mask_format="polygon"):
    ...
    try:
      ...
      boxes_pre = (
        np.stack(
            [BoxMode.convert(obj["bbox_pre"], obj["bbox_mode"], BoxMode.XYXY_ABS) for obj in annos]
        )
        if len(annos)
        else np.zeros((0, 4))
      )
    ...
    try:
      classes_3 = [int(obj["category_id_3"]) for obj in annos]
      classes_3 = torch.tensor(classes_3, dtype=torch.int64)
      target.gt_classes_3 = classes_3
    except:
      pass

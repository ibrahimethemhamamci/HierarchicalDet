...
class EvalHook(HookBase):
    ...
    def __init__(self, eval_period, eval_function, i, eval_after_train=True):
        """
        Args:
    ...
    def _do_eval(self):
        ...
        results = self._func(self.i)

        if results:
            for result in results:
              assert isinstance(
                  result, dict
              ), "Eval function must return a dict. Got {} instead.".format(result)
              """
              assert isinstance(
                  results[1], dict
              ...
              flattened_results_1 = flatten_results_dict(result)
              for k, v in flattened_results_1.items():
                  try:
                      v = float(v)
              ...
              self.trainer.storage.put_scalars(**flattened_results_1, smoothing_hint=False)
            
              """
              
              flattened_results_2 = flatten_results_dict(results[1])
              for k, v in flattened_results_1.items():
                  try:
                      v = float(v)
                  except Exception as e:
              ...
              self.trainer.storage.put_scalars(**flattened_results_2, smoothing_hint=False)
              
              
              
              flattened_results_3 = flatten_results_dict(results[2])
              for k, v in flattened_results_3.items():
                  try:
                      v = float(v)
                  except Exception as e:
                      raise ValueError(
              ...
              self.trainer.storage.put_scalars(**flattened_results_3, smoothing_hint=False)
            """
